...
public class EmailService(IConfiguration configuration, ILogger<EmailService> logger) : IEmailService
{
    public async Task SendEmailVerificationAsync(string email, string username, string token)
    {
        ...
        var body = $@"
            ...
            <a href='{verificationUrl}' style='background-color: #123a65; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;'>
                Verificar Correo
            </a>
    ...
    public async Task SendPasswordResetAsync(string email, string username, string token)
    {
        ...
        var body = $@"
            ...
            <a href='{resetUrl}' style='background-color: #75212a; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;'>
    ...
    {
        var subject = "¡Bienvenido a Kinal Eats App!";

        var body = $@"
            <h2>¡Bienvenido a Kinal Eats App, {username}!</h2>
    ...
    private async Task SendEmailAsync(string to, string subject, string body)
    {
        ...
        try
        {
            ...
            if (!enabled)
            {
                logger.LogInformation("Correo electrónico deshabilitado en la configuración. Omitiendo envío");
                return;
            }
            ...
            {
                logger.LogError("La configuración SMTP no está correctamente configurada");
                throw new InvalidOperationException("La configuración SMTP no está correctamente configurada");
            }
            ...
            try
            {
                ...
                logger.LogInformation("Correo electrónico enviado exitosamente");

                await client.DisconnectAsync(true);
                logger.LogInformation("Pipeline de correo electrónico completado");
            }
            catch (MailKit.Security.AuthenticationException authEx)
            {
                logger.LogError(authEx, "Falló la autenticación de Gmail. Verifica la contraseña de aplicación.");
                throw new InvalidOperationException($"Falló la autenticación de Gmail: {authEx.Message}. Por favor verifica tu contraseña de aplicación.", authEx);
            }
            catch (Exception ex)
            {
                logger.LogError(ex, "Error al enviar el correo electrónico");
                throw;
            }
            logger.LogInformation("Correo electrónico procesado");
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Error al enviar el correo electrónico");
            ...
            if (useFallback)
            {
                logger.LogWarning("Usando respaldo de correo electrónico");
