    ...
    ILogger<AuthService> logger) : IAuthService
{
    ...
    public async Task<RegisterResponseDto> RegisterAsync(RegisterDto registerDto)
    {
        ...
        if (await userRepository.ExistsByUsernameAsync(registerDto.Username))
        {
            ...
            throw new BusinessException(ErrorCodes.USERNAME_ALREADY_EXIST, "Username already exists");
        }
        ...
        if (registerDto.ProfilePicture != null && registerDto.ProfilePicture.Size > 0)
        {
            ...
            try
            {
                ...
                logger.LogInformation("Intentando subir imagen: {FileName}, Tamaño: {Size} bytes", fileName, registerDto.ProfilePicture.Size);

                profilePicturePath = await _cloudinaryService.UploadImageAsync(registerDto.ProfilePicture, fileName);

                logger.LogInformation("Imagen subida exitosamente: {Path}", profilePicturePath);
            }
            catch (Exception ex)
            {
                logger.LogError(ex, "Error detallado al subir imagen. Mensaje: {Message}, Tipo: {Type}",
                    ex.Message, ex.GetType().Name);
                throw new BusinessException(ErrorCodes.IMAGE_UPLOAD_FAILED, $"Failed to upload profile image: {ex.Message}");
            }
        }
        else
        {
            profilePicturePath = _cloudinaryService.GetDefaultAvantarUrl();
        }
        ...
        var user = new User
        {
            ...
            UserRoles =
            [
                new global::AuthService.Domain.Entities.UserRole
                {
                    Id = userRoleId,
                    UserId = userId,
        ...
        return new RegisterResponseDto
        {
            ...
            IsEmailVerificationRequerid = true
        };
    }
    ...
    public async Task<AuthResponseDto> LoginAsync(LoginDto loginDto)
    {
        ...
        return new AuthResponseDto
        {
            ...
            Token = token,
            UserDetailsDto = MapToUserDetailsDto(user),
    ...
    public async Task<EmailResponseDto> ForgotPasswordAsync(ForgotPasswordDto forgotPasswordDto)
    {
        ...
        var resetToken = TokenGeneratorService.GeneratePassowrdResetToken();
